...
public class KafkaSource implements IStreamSourceGener<Table> {
	...
	/**
	 * 此方法根据传入的SourceTableInfo内容，创建并获取一个Kafka数据源的表。
	 * 如果用户没有指定auto.offset.reset参数，默认会使用Kafka的groupoffset。
	 *
	 * @param sourceTableInfo 包含Kafka连接信息和表定义信息
	 * @return 返回根据Kafka数据源构建的Flink Table
	 */
	...
	public Table genStreamSource(SourceTableInfo sourceTableInfo, StreamExecutionEnvironment env, StreamTableEnvironment tableEnv) {
		...
		// 将SourceTableInfo强制转换成KafkaSourceTableInfo来获取Kafka相关参数。
