...
public class LauncherOptionParser {
    ...
    //add by lixiyuan
    //是否需要检测语法
    public static final String CHECK_SQL="checkSQL";
    ...
    public LauncherOptionParser(String[] args) throws Exception {
        ...
        //add by lixiyuan
        options.addOption(OPTION_QUEUE,true,"yarn queue");
        options.addOption(CHECK_SQL,true,"Is the current operation an operation that detects the SQL syntax");
        ...
        String checksql = cl.getOptionValue(CHECK_SQL);
        if(StringUtils.isNotBlank(checksql)){
            properties.setChecksql(checksql);
        }
    }
